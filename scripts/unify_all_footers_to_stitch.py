            ...
            if f.endswith('.html') and not f.startswith('google') and f != 'index.html':
                ...
                if pattern.search(content):
                    ...
                    with open(p, 'w', encoding='utf-8') as file:
                        file.write(content)
                    count += 1
                elif '<footer class="stitch-footer' not in content:
                    # Check if there's any other footer
                    any_footer = re.compile(r'<footer[^>]*>.*?</footer>', re.DOTALL)
                    if any_footer.search(content):
                        content = any_footer.sub(STITCH_FOOTER_HTML, content)
                        with open(p, 'w', encoding='utf-8') as file:
                            file.write(content)
                        count += 1

    print(f"Successfully replaced and unified footers across {count} HTML pages to match landing page!")
